...
if(jQuery) (function($) {
  ...
  function show(type, message, options) {
    ...
    // Create elements
    style = options.style; //Added 7Dec2022
    ...
    //Center'A
    // Debug.log("Alertable:: Center "+$(modal).height());
    $(modal).center();
    // Watch for submit
    $(modal).on('submit.alertable', function(event) {
      ...
      if(type === 'prompt') {
        ...
        for(i = 0; i < formData.length; i++) {
          logTest(formData[i].name+ " :: " + formData[i].value);
          values[formData[i].name] = formData[i].value;
        }

        // values['newimage[]'] = $('input[name="newimage"]')[0].files;
        // var data = new FormData(this);
        // values['newimage'] = data;
        // logTest(values);
      } else {
        values = null;
      }
  ...
  $.alertable = {
    ...
    defaults: {
      ...
      style: '',
      modal:
        '<form class="alertable" enctype="multipart/form-data">' +
